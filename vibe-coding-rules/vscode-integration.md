...
**Principles:**
- Activate only when needed (`onCustomEditor:speckitForHumans.editor`)
...
"commands": [
  {
    "command": "speckitForHumans.openFile",
    "title": "Open with Speckit for Humans",
    "category": "Speckit for Humans"  // Always categorize
  }
]

// In extension.ts
vscode.commands.registerCommand('speckitForHumans.openFile', () => {
  // Implementation
});
```
...
**Palette Hygiene:** Prefer toolbar/inline affordances. If palette entry needed, scope with `when: activeCustomEditorId == speckitForHumans.editor`
...
2. Read: `vscode.workspace.getConfiguration('speckitForHumans')`
