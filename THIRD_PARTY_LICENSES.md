# Third-Party Licenses

This document lists the third-party open source software components used in Speckit for Humans and their respective licenses.

## Dependencies

### @tiptap/core, @tiptap/markdown, @tiptap/pm, @tiptap/starter-kit
- **License:** MIT
- **Copyright:** © 2024 Tiptap Inc.
- **Repository:** https://github.com/ueberdosis/tiptap
- **License URL:** https://github.com/ueberdosis/tiptap/blob/main/LICENSE

### @tiptap/extension-code-block-lowlight
- **License:** MIT
- **Copyright:** © 2024 Tiptap Inc.
- **Repository:** https://github.com/ueberdosis/tiptap
- **License URL:** https://github.com/ueberdosis/tiptap/blob/main/LICENSE

### @tiptap/extension-image, @tiptap/extension-link, @tiptap/extension-list, @tiptap/extension-table
- **License:** MIT
- **Copyright:** © 2024 Tiptap Inc.
- **Repository:** https://github.com/ueberdosis/tiptap
- **License URL:** https://github.com/ueberdosis/tiptap/blob/main/LICENSE

### cheerio
- **License:** MIT
- **Copyright:** © 2024 cheeriojs
- **Repository:** https://github.com/cheeriojs/cheerio
- **License URL:** https://github.com/cheeriojs/cheerio/blob/main/LICENSE

### docx
- **License:** MIT
- **Copyright:** © 2024 Dolan Miu
- **Repository:** https://github.com/dolanmiu/docx
- **License URL:** https://github.com/dolanmiu/docx/blob/master/LICENSE

### highlight.js
- **License:** BSD 3-Clause
- **Copyright:** © 2006-2024, Ivan Sagalaev
- **Repository:** https://github.com/highlightjs/highlight.js
- **License URL:** https://github.com/highlightjs/highlight.js/blob/main/LICENSE

### image-size
- **License:** MIT
- **Copyright:** © 2014-2024 Aditya Yadav
- **Repository:** https://github.com/image-size/image-size
- **License URL:** https://github.com/image-size/image-size/blob/main/LICENSE

### katex
- **License:** MIT
- **Copyright:** © 2013-2024 Khan Academy
- **Repository:** https://github.com/KaTeX/KaTeX
- **License URL:** https://github.com/KaTeX/KaTeX/blob/main/LICENSE

### lowlight
- **License:** MIT
- **Copyright:** © 2024 Titus Wormer
- **Repository:** https://github.com/wooorm/lowlight
- **License URL:** https://github.com/wooorm/lowlight/blob/main/license

### markdown-it
- **License:** MIT
- **Copyright:** © 2014-2024 Vitaly Puzrin, Alex Kocharin
- **Repository:** https://github.com/markdown-it/markdown-it
- **License URL:** https://github.com/markdown-it/markdown-it/blob/master/LICENSE

### mermaid
- **License:** MIT
- **Copyright:** © 2024 Mermaid contributors
- **Repository:** https://github.com/mermaidjs/mermaid
- **License URL:** https://github.com/mermaidjs/mermaid/blob/develop/LICENSE

### turndown
- **License:** MIT
- **Copyright:** © 2024 Dom Christie
- **Repository:** https://github.com/mixmark-io/turndown
- **License URL:** https://github.com/mixmark-io/turndown/blob/master/LICENSE

## Development Dependencies

### @types/node, @types/vscode, @types/jest, @types/image-size, @types/turndown
- **License:** MIT
- **Copyright:** © Microsoft Corporation and contributors
- **Repository:** https://github.com/DefinitelyTyped/DefinitelyTyped
- **License URL:** https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/LICENSE

### @eslint/eslintrc
- **License:** MIT
- **Copyright:** © OpenJS Foundation and contributors
- **Repository:** https://github.com/eslint/eslintrc
- **License URL:** https://github.com/eslint/eslintrc/blob/main/LICENSE

### @typescript-eslint/eslint-plugin, @typescript-eslint/parser
- **License:** MIT
- **Copyright:** © TypeScript ESLint contributors
- **Repository:** https://github.com/typescript-eslint/typescript-eslint
- **License URL:** https://github.com/typescript-eslint/typescript-eslint/blob/main/LICENSE

### @vscode/vsce
- **License:** MIT
- **Copyright:** © Microsoft Corporation
- **Repository:** https://github.com/microsoft/vscode-vsce
- **License URL:** https://github.com/microsoft/vscode-vsce/blob/main/LICENSE

### concurrently
- **License:** MIT
- **Copyright:** © 2014-2024 Kim Kulling
- **Repository:** https://github.com/open-cli-tools/concurrently
- **License URL:** https://github.com/open-cli-tools/concurrently/blob/main/LICENSE

### esbuild
- **License:** MIT
- **Copyright:** © 2020-2024 Evan Wallace
- **Repository:** https://github.com/evanw/esbuild
- **License URL:** https://github.com/evanw/esbuild/blob/main/LICENSE.md

### eslint, eslint-config-prettier, eslint-plugin-prettier
- **License:** MIT
- **Copyright:** © ESLint contributors, Prettier contributors
- **Repositories:** 
  - https://github.com/eslint/eslint
  - https://github.com/prettier/eslint-config-prettier
  - https://github.com/prettier/eslint-plugin-prettier
- **License URLs:** See respective package repositories

### jest, jest-environment-jsdom, ts-jest
- **License:** MIT
- **Copyright:** © Meta Platforms, Inc. and affiliates, Facebook, Inc.
- **Repositories:**
  - https://github.com/jestjs/jest
  - https://github.com/jestjs/jest/tree/main/packages/jest-environment-jsdom
  - https://github.com/kulshekhar/ts-jest
- **License URLs:** See respective package repositories

### prettier
- **License:** MIT
- **Copyright:** © James Long and contributors
- **Repository:** https://github.com/prettier/prettier
- **License URL:** https://github.com/prettier/prettier/blob/main/LICENSE

### typescript
- **License:** Apache-2.0
- **Copyright:** © Microsoft Corporation
- **Repository:** https://github.com/microsoft/TypeScript
- **License URL:** https://github.com/microsoft/TypeScript/blob/main/LICENSE.txt

## Assets

### codicon.css, codicon.ttf
- **License:** MIT
- **Copyright:** © Microsoft Corporation
- **Source:** VS Code Extension API
- **Note:** These are Microsoft's Codicon font files used for icons in the extension UI.

---

**Note:** This list is provided for informational purposes. For the complete and authoritative license text, please refer to the respective package repositories or the `node_modules` directory after installation.
