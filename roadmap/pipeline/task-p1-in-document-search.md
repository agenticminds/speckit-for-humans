...
- **Task name:** Reliable in-document search for Speckit for Humans
...
- **Problem:** In the Speckit for Humans custom editor, users cannot reliably search within the current document; pressing Cmd/Ctrl+F does not produce a working in-document search experience.
...
- **Success criteria:**
  - In a Speckit for Humans editor, pressing Cmd/Ctrl+F always opens a search experience that clearly highlights matches in the WYSIWYG content and lets users move between them (next/previous).
...
- **Entry points:**
  - Keyboard: Cmd+F / Ctrl+F when the Speckit for Humans editor is focused.
  - Optional toolbar icon: a Find in document button in the WYSIWYG toolbar that triggers the same search experience.
  - Optional command palette action: `Speckit for Humans: Find in Document` that focuses the editor and opens search.
- **Flow:**
  - User is editing in Speckit for Humans and presses Cmd/Ctrl+F.
...
- **Current behavior (user-facing):** Inside the Speckit for Humans editor, there is no reliable in-document search experience; Cmd/Ctrl+F does not consistently activate a usable find widget, and there is no dedicated search control in the toolbar.
